...
type Merchant = { brand: string; name: string; logo: string; phone: string; email: string; address: string }
type Info = { paid: boolean; amount: number; customerName: string; orderNumber: string; merchant?: Merchant }
...
export default function PaymentSuccessPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/payment-link/session?session_id=${encodeURIComponent(sid)}`)
      .then((r) => r.json())
      .then((j) => { if (!j.error) { setInfo(j); if (j.merchant?.name) document.title = `Payment received — ${j.merchant.name}` } })
      .catch(() => {})
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f4f3f1', fontFamily: 'system-ui, -apple-system, "Segoe UI", sans-serif', color: '#1a1a1a', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: '44px 36px', maxWidth: 460, width: '100%', textAlign: 'center', boxShadow: '0 1px 3px rgba(0,0,0,.08)' }}>
        ...
        {merchant?.logo ? <img src={merchant.logo} alt={merchant.name} style={{ height: 26, marginBottom: 28 }} /> : <div style={{ height: 26, marginBottom: 28 }} />}
        ...
        <p style={{ color: '#6b6b6b', fontSize: 15, margin: '0 0 24px' }}>
          Thank you{info?.customerName ? `, ${info.customerName}` : ''}. Your payment{merchant?.name ? ` to ${merchant.name}` : ''} has been processed successfully.
        </p>
        ...
        {merchant && (merchant.name || merchant.address || merchant.phone || merchant.email) ? (
          <div style={{ borderTop: '1px solid #eceae6', marginTop: 28, paddingTop: 20, color: '#6b6b6b', fontSize: 13, lineHeight: 1.6 }}>
            {merchant.name ? <div style={{ fontWeight: 600, color: '#1a1a1a' }}>{merchant.name}</div> : null}
            {merchant.address ? <div>{merchant.address}</div> : null}
            {merchant.phone ? <div>Ph: <a href={`tel:${merchant.phone.replace(/\s+/g, '')}`} style={{ color: '#1a1a1a' }}>{merchant.phone}</a></div> : null}
            {merchant.email ? <div><a href={`mailto:${merchant.email}`} style={{ color: '#1a1a1a' }}>{merchant.email}</a></div> : null}
          </div>
        ) : null}
      </div>
    </div>
