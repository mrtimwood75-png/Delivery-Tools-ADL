...
def extract_value_before_unit(parts, unit_label: str) -> str:
    unit_label = unit_label.strip().lower()
    for i, part in enumerate(parts):
        if str(part).strip().lower() == unit_label and i > 0:
            return str(parts[i - 1]).strip().replace(",", ".")
    return ""
...
def extract_new_format_item_notes(extra_lines):
    ...
    for line in extra_lines:
        ...
        if line.lower().startswith("total volume"):
            continue
        if line.lower().startswith("total weight"):
            continue
...
def parse_new_format_item_row(parts):
    ...
    volume_value = extract_value_before_unit(parts, "Cu M")
    weight_value = extract_value_before_unit(parts, "Kg")
